    ...
    >
      <div className="mx-auto flex h-16 max-w-[1600px] items-center justify-between px-8">
        ...
        >
          <span className="relative flex h-9 w-9 items-center justify-center overflow-hidden rounded-xl bg-slate-900 shadow-sm transition-transform duration-300 group-hover:scale-105 dark:bg-slate-50">
            ...
            <span className="pointer-events-none absolute -right-3 -top-3 h-6 w-6 rounded-full bg-amber-400/30 blur-md transition-opacity duration-500 group-hover:opacity-100 opacity-0" />
          </span>
        ...
        <div className="flex items-center gap-6">
          ...
          <span aria-hidden="true" className="hidden h-6 w-px bg-slate-200 md:block dark:bg-slate-800" />

          {/* Right side actions */}
          <div className="flex items-center gap-1">
          <Button variant="ghost" size="icon" asChild className="text-slate-900 dark:text-slate-50">
            <Link href="/search">
              ...
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <Button variant="ghost" size="icon" className="relative" aria-label="Account menu">
                    <div className="flex h-7 w-7 items-center justify-center rounded-full bg-slate-900 text-xs font-semibold text-white dark:bg-slate-50 dark:text-slate-900" aria-hidden="true">
      ...
      {mobileMenuOpen && (
        <div className="border-t md:hidden">
          <nav aria-label="Mobile navigation" className="mx-auto flex max-w-7xl flex-col gap-2 px-6 py-4">
            {NAV_LINKS.map((link) => {
              const isActive = pathname === link.href || pathname.startsWith(link.href + '/');
              return (
                <Link
            ...
            {user && (
              <>
                ...
                <Link href="/account" className="px-2 py-1.5 text-sm font-medium" onClick={() => setMobileMenuOpen(false)}>
                  Account
                </Link>
                <Link href="/bookings" className="px-2 py-1.5 text-sm font-medium" onClick={() => setMobileMenuOpen(false)}>
                  My Bookings
                </Link>
                <button
