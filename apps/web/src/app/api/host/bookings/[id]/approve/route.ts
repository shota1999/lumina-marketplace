...
export async function POST(request: NextRequest, { params }: RouteParams) {
  ...
  const log = createRequestLogger({ requestId, route: 'POST /api/host/bookings/:id/approve' });
    ...
    return withSpan(
      ...
      async () => {
        ...
        if (row.status === 'confirmed') {
          log.done(200);
          return successResponse({ approved: true, alreadyConfirmed: true });
        }
        if (row.status !== 'pending') {
          ...
          return errorResponse(
            {
              code: 'INVALID_STATUS',
              message: `Cannot approve a booking with status "${row.status}"`,
            },
            400,
          );
        }
        ...
        audit({
          action: 'booking.approved_by_host',
          userId: user.id,
          resourceId: id,
          requestId,
        });
        log.info('Host approved booking', { hostId: user.id, bookingId: id });
        log.done(200);
        return successResponse({ approved: true });
      },
    );
  } catch (error) {
    log.error('Approve failed', { error: String(error) });
    log.done(500);
    return errorResponse({ code: 'INTERNAL_ERROR', message: 'Failed to approve booking' }, 500);
  }
}
